...
- **Tech Stack**: Python 3.10+, PyTorch, CUDA, OpenCV, Docker, Hugging Face Transformers
- **Main File**: app.py - Gradio/FastAPI web application for video processing
- **Core Mechanic**: AI video generation with synchronized Foley audio effects
- **Key Integration**: HunyuanVideo model, Hugging Face model hub, RunPod GPU cloud
- **Platform Support**: RunPod, Docker containers, NVIDIA GPU infrastructure
- **DO NOT**: Commit model weights, hardcode API keys, expose internal ports
